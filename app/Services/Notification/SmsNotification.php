    ...
    public function sendAgain(SmsBatch $smsBatch): void
    {
        ...
        foreach ($this->chuncks() as $notification) {
            switch ($smsBatch->type) {
                case 'advertise':
                    (new PeerToPeer($smsBatch->pattern, $notification, $this->category, $uniqueCode))->isAdmin($this->isAdmin)->isSystematic($this->isSystematic)->sendAgain($this->userData,
                        $smsBatch);
                    break;
                case 'simcard'  :
                    ...
                    break;
                case 'service':
                    (new PeerToPeer($smsBatch->pattern, $notification, $this->category, $uniqueCode, 'service'))->isAdmin($this->isAdmin)->isSystematic($this->isSystematic)->sendAgain($this->userData,
                        $smsBatch);
                    break;
            }
        }
